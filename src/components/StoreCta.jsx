function StoreCta() {
  return (
    <section id="tienda" className="mx-auto max-w-7xl px-6 pb-16 pt-4 lg:px-10 lg:pb-20">
      <div className="flex flex-col gap-6 rounded-lg border border-[var(--color-line)] bg-[var(--color-sky)] p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
        <div>
        ...
        <a href="#tienda" aria-label="Tienda próximamente" className="shrink-0 rounded-md border border-[var(--color-primary)] bg-white px-5 py-3 text-center text-sm font-semibold text-[var(--color-primary)] transition hover:bg-[var(--color-primary)] hover:text-white">Ir a la tienda <span aria-hidden="true">↗</span></a>
      </div>
    </section>
  )
}
